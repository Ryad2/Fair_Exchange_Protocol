...
async function main() {
    ...
    console.log("🧪 TEST COMPLET - GATE 1 (Force état + submitCommitmentLeft)");
    console.log("=".repeat(80));
    ...
    // ============================================
    // FORCER L'ÉTAT À WaitVendorDataLeft (6) avec chall = 1
    // ============================================
    console.log("🔄 FORCEMENT DE L'ÉTAT À WaitVendorDataLeft (6) avec chall = 1...");
    console.log("─".repeat(50));
    ...
    while (state !== 6 && round < maxRounds) {
        ...
        if (state === 3) { // ChallengeBuyer
            // Le buyer doit répondre (même si a == b, le buyer doit répondre avant giveOpinion)
            console.log(`  📤 Buyer répond au challenge ${chall}...`);
            ...
            try {
                await dispute.connect(buyerToUse).respondChallenge(responseHex);
                console.log(`  ✅ Réponse envoyée`);
            } catch (error: any) {
                console.log(`  ⚠️  Erreur lors de respondChallenge: ${error.message}`);
                // Si a == b et que respondChallenge échoue, peut-être que buyerResponses[chall] est déjà défini
                // Vérifier si buyerResponses[chall] est déjà défini
                const existingResponse = await dispute.getBuyerResponse(chall);
                if (existingResponse !== ethers.ZeroHash) {
                    console.log(`  ℹ️  buyerResponses[${chall}] est déjà défini, on passe à giveOpinion`);
        ...
        if (state === 2) { // WaitVendorOpinion
            // Le vendor doit donner son opinion (false pour forcer left)
            console.log(`  📤 Vendor donne son opinion (false pour forcer left)...`);
            const vendorToUse = vendorSignerWallet || vendorWallet;
            if (!vendorToUse) {
                throw new Error("❌ Impossible de trouver le vendor signer");
            }
            await dispute.connect(vendorToUse).giveOpinion(false);
            console.log(`  ✅ Opinion envoyée`);
        } else {
    ...
    console.log("=".repeat(80));
    console.log("\n📝 Résumé:");
    console.log("  ✅ État forcé à WaitVendorDataLeft (6) avec chall = 1");
